  ...
  <section className="py-20 px-5 sm:px-10 gradient-hero relative overflow-hidden">
    ...
    <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-10 items-center relative">
      <div>
        <div className="flex flex-wrap gap-8 mb-7">
          {[
            ...
            { n: "25k", s: "+", l: "Surgeries Done" },
            { n: "98", s: "%", l: "Success Rate" },
          ].map((it) => (
            <div key={it.l}>
              <div className="font-display text-[clamp(36px,5vw,58px)] text-white font-bold leading-none">
                {it.n}<span className="text-[0.5em] text-sky-300">{it.s}</span>
              </div>
              <div className="text-xs text-white/60 uppercase tracking-wider mt-1">{it.l}</div>
            </div>
          ))}
        </div>
        <h2 className="font-display text-[clamp(26px,3.5vw,40px)] text-white leading-tight mb-3">
          15 Years of Trusted<br />Eye Care Excellence
        </h2>
        <p className="text-white/75 leading-relaxed max-w-lg">
          Trusted by thousands of families across Hyderabad. Book your consultation today and take
      ...
      <div className="flex flex-col items-start gap-4 lg:items-end">
        <button
          onClick={onBookClick}
          className="inline-flex items-center gap-3 bg-white text-navy px-9 py-4 rounded-full text-base font-bold shadow-soft hover:-translate-y-0.5 transition"
        >
        ...
        <p className="text-white/55 text-xs">Available Mon–Sat | 9 AM – 6 PM</p>
      </div>
    </div>
  </section>
);

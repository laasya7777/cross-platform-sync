    ...
    >
      {SLIDES.map((s, idx) => (
        ...
        >
          <img
            src={s.img}
            alt={s.label}
            className={`w-full h-full object-cover transition-transform ease-out ${idx === i ? "scale-100 duration-[6000ms]" : "scale-110 duration-0"}`}
      ...
      <div className="relative z-10 h-full flex items-center px-5 sm:px-10 lg:px-20">
        <div className="max-w-2xl text-white">
          ...
          <h1 className="font-display text-[clamp(34px,5.5vw,62px)] leading-[1.1] mb-4 animate-fade-up" style={{ animationDelay: "0.15s" }}>
            Restoring <span className="text-sky-300">Vision</span>,
            <br />Transforming Lives
          </h1>
          <p className="text-base sm:text-lg text-white/75 leading-relaxed mb-8 max-w-xl animate-fade-up" style={{ animationDelay: "0.3s" }}>
            Fellowship-trained ophthalmologist offering world-class cataract, LASIK, glaucoma and retina care — backed by 25,000+ successful surgeries.
          </p>
          <div className="flex flex-wrap gap-3 animate-fade-up" style={{ animationDelay: "0.45s" }}>
            <button
              onClick={onBookClick}
              className="inline-flex items-center gap-2 bg-gradient-to-r from-sky to-accent text-white px-7 py-3 rounded-full text-sm font-semibold hover:-translate-y-0.5 transition shadow-glow"
            >
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-20">
        {SLIDES.map((_, idx) => (
          <button
            key={idx}
            onClick={() => setI(idx)}
            className={`h-2 rounded-full transition-all ${idx === i ? "bg-white w-7" : "bg-white/40 w-2"}`}
